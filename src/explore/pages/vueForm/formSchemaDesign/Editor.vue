...
// import componentWithDialog from '../designDialog';
...
export default {
  ...
  methods: {
    ...
    handlePreview() {
      ...
      //       // fetch('baseUrl').formData(this.publishUrl, {
      //       //   "fields": JSON.stringify(props, null ,0),
      //       //   "formData": JSON.stringify(data, null, 0)
      //       // }).then( (resp) => {
      //       //   alert(resp.msg)
      //       //   window.close()
      //       // })
    ...
    changeSchema(code) {
      try {
        ...
        if (formConfig.formProps){
          if(!this.formConfig.formProps) this.formConfig.formProps = {}
          Object.assign(this.formConfig.formProps, formConfig.formProps);
        }
        if (formConfig.formFooter){
          if(!this.formConfig.formFooter) this.formConfig.formFooter = {}
          Object.assign(this.formConfig.formFooter, formConfig.formFooter);
        }
    ...
    handleToDemo() {
      ...
      this.openNewPage(link, '_specialViewForm');
    },
    openNewPage(url, target = '_blank') {
...
<style module>
    ...
    .closeToolbar .toolBarWrap {
      left: calc(0 - var(--tool-bar-width));
    }

    .closeToolbar .toolBarWrap .leftCaret{
      right: -18px;
    }

    .closeToolbar .toolBarWrap .el-icon-caret-right {
      transform: rotate(0);
    }
    .contentBox {
        position: relative;
